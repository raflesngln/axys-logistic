...
function print_(ctk=''){
    var track   = $('#statusproses1').val();
      if(track==' '){
         swal("Oops... Last Status Tacking is required", "Please Input Last Status Tacking!...", "error");
         exit();
      }
      
    var ctype   = $('#statusproses2').val();
      if(ctype==' '){
         swal("Oops... Which Contain Status Tracking is required", "Please Input Which Contain Status Tracking!...", "error");
         exit();
      } 
    ...
    switch (ctk) {
    case '1':
        cetakPrev('4|'+tgl1+'|'+tgl2+'|'+track+'|'+ctype+'|'+vper,"<?php echo site_url('cas/Ccas_rpt/rpt_last_track_countain')?>","#preview_his"); 
        UIkit.switcher('#h_tab').show(1);
        break;
    case '2':
    //alert(ctk);
        cetakTri('1|'+tgl1+'|'+tgl2+'|'+track+'|'+ctype+'|'+vper,"<?php echo site_url('cas/Ccas_rpt/rpt_last_track_countain')?>");     
        break;
    case '3':
    //alert(ctk);
        cetakTri('2|'+tgl1+'|'+tgl2+'|'+track+'|'+ctype+'|'+vper,"<?php echo site_url('cas/Ccas_rpt/rpt_last_track_countain')?>"); 
        break;
    }
}
    ...
    <div class="uk-width-medium-1-1">
        <div class="md-card" id="cont">
            <div class="md-card-toolbar" style="height: 55px;">
                ...
                <h3 id="title" class="md-card-toolbar-heading-text">REPORT TRACKING WHITE OPERAND</h3>
            </div>
        ...
        <div class="uk-grid" >
             <div class="uk-width-medium-1-3 uk-width-large-1-3 "> 
                 <span class="icheck-inline">
                      ...
                      <label for="radio_demo_inline_1" class="inline-label">All</label>
                 </span>
                 <span class="icheck-inline">
                      <input type="radio" name="radio_demo_inline" id="radio_demo_inline_2" onchange="perval(2);"/>
                      <label for="radio_demo_inline_2" class="inline-label">Contains</label>
                 </span>
                 <span class="icheck-inline">
                      <input type="radio" name="radio_demo_inline" id="radio_demo_inline_2" onchange="perval(3);"/>
                      <label for="radio_demo_inline_2" class="inline-label">Not Contains</label>
                 </span>
             </div>
        </div>
        <div class="uk-grid" >
            <div class="uk-width-large-1-3" >
                <div class="uk-form-row parsley-row" id="Sts_1">
                    <label for="">Contains Status Tracking<span class="req"> *</span></label>
                    <select id="statusproses1" name="statusproses1" class="statusproses" required multiple>
                       <option value=" ">...</option>
                    </select>
                </div>
            </div>
             <div class="uk-width-medium-1-3 uk-width-large-1-3" id="Sts_2">
                <div class="uk-form-row parsley-row">
                    <label for="">Not Contains Status Tracking<span class="req"> *</span></label>
                    <select id="statusproses2" name="statusproses2" class="statusproses" required multiple>
                       <option value=" ">...</option>
                    </select>
                </div>
             </div>
