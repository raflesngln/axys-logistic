...
function respon_bc($tanggal='',$tabel='',$mawb='',$txtsearch=''){
	//set_time_limit (300)
   ...
   ini_set('memory_limit', '-1');
	  ...
	  if($mawb=='all'){
		  ...
		  $cari_awb=rtrim($cari_awb,",");
		  $query_custom="AND mawb IN($cari_awb)";
		  $judul='Custom_mawb';
	  }
...
//----------------------CREATE COLOUMN HEADER------------------------------------------------------------------------
		$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Release_&_Non-Release_'.$judul.'_'.date('d-m-Y',strtotime($tanggal)));
		...
		$objPHPExcel->getActiveSheet()->setCellValue('E3',  'Flight Date');
		$objPHPExcel->getActiveSheet()->setCellValue('F3',  'Consignee Name');
		$objPHPExcel->getActiveSheet()->setCellValue('G3',  'Consignee NPWP');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('K3',  'Kind of Good');
		$objPHPExcel->getActiveSheet()->setCellValue('L3',  'Hs Code');
		$objPHPExcel->getActiveSheet()->setCellValue('M3', 'CIF');
		$objPHPExcel->getActiveSheet()->setCellValue('N3',  'Bm Persen');
		$objPHPExcel->getActiveSheet()->setCellValue('O3',  'Bm Value');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('U3',  'Status');
		$objPHPExcel->getActiveSheet()->setCellValue('V3',  'Description');
		$objPHPExcel->getActiveSheet()->setCellValue('W3',  'Date REKAM');
		$objPHPExcel->getActiveSheet()->setCellValue('X3',  'Time REKAM');

		//make a border column
		$objPHPExcel->getActiveSheet()->getStyle('B3:X3')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		//-----------------------------------END create column HEADER------------------------------------------


	   $result=$this->Model_gtln->getCustom("*",$tabel,
			  "WHERE flight_date ='$tanggal' $query_custom ");
        $no     = 0;                                  
        ...
        foreach ($result as $row){
           $no=$no+1;
           $hawb=$row->hawb;
           $kdrespon = 'Tidak ada Respon BC';
           $wkrespon ='';
		   $wk_time='';
		   $wk_ket='';
		
		$url = "http://www.beacukai.go.id/barangkiriman.html";
		
		//for($i=0;$i < count($data_hawb);$i++){
			//$hawb=$data_hawb[$i];
			$curlHandle = curl_init();
			curl_setopt($curlHandle, CURLOPT_URL, $url);
			curl_setopt($curlHandle, CURLOPT_POSTFIELDS, "txtNpwpPjt=020640181428000&txtNoBarang=$hawb");
			curl_setopt($curlHandle, CURLOPT_HEADER, 0);
			curl_setopt($curlHandle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curlHandle, CURLOPT_TIMEOUT,30);
			curl_setopt($curlHandle, CURLOPT_POST, 1);
			$data_res=curl_exec($curlHandle);
			curl_close($curlHandle);
			
			$is_data= strlen($data_res);
			$bc_kd_respon='';
			$bc_kd_ket='';
			$bc_wk_rekam='';
			$bc_tgl_rekam='';
			if($is_data > 150){
             ...
             $response=explode('<table class="table table-striped table-bordered">',$data_res);              
             ...
             $arrlength = count($response);
                    ...
                    $dat=str_replace( '<h3 class="panel-title">TAGIHAN BILLING</h3>',"",$dat);
					 ...
					 //echo $hawb.' - '.$bc_kd_respon.'-'.$bc_kd_ket.' - '.$bc_wk_rekam.'-'.$bc_tgl_rekam.'<br>';
// if($wkrespon !=''){
				...
				$objPHPExcel->getActiveSheet()->setCellValue('U'.$kol, $bc_kd_respon);
				$objPHPExcel->getActiveSheet()->setCellValue('V'.$kol, $bc_kd_ket);
				$objPHPExcel->getActiveSheet()->setCellValue('W'.$kol, $bc_wk_rekam);
				$objPHPExcel->getActiveSheet()->setCellValue('X'.$kol, $bc_tgl_rekam);
				$kol++;
				$nomor++;
			 //  }
		...
		$objWorksheet->getStyle('B3:X3')->applyFromArray($fontHeader);
		$objWorksheet->getStyle('B2:H2')->applyFromArray($font_title);
		
		// Rename worksheet name
		$objPHPExcel->getActiveSheet()->setTitle('Release&Non_Release_'.$tanggal);
		...
		header('Content-Disposition: attachment;filename="M_Release_&_Non_Release_'.$judul.'_'.$tanggal.'.xlsx"');
 ...
 function non_release_excel($tanggal='',$tabel='',$mawb='',$txtsearch=''){	
 	   ini_set('memory_limit', '-1');
	  $pecah=explode("\n",$txtsearch);
	  if($mawb=='all'){
		   $query_custom='';
...
//----------------------CREATE COLOUMN HEADER------------------------------------------------------------------------
		...
		$objPHPExcel->getActiveSheet()->setCellValue('U3',  'Status');
		$objPHPExcel->getActiveSheet()->setCellValue('V3',  'Description');
		$objPHPExcel->getActiveSheet()->setCellValue('W3',  'WK_REKAM');
		$objPHPExcel->getActiveSheet()->setCellValue('X3',  'WK_RESPON');
       ...
       $result=$this->Model_gtln->getCustom("*",$tabel,
			  "WHERE flight_date ='$tanggal' $query_custom GROUP BY hawb");
        $no     = 0;                                  
        ...
        foreach ($result as $row)
        {
		   ...
		   $wk_time='';
		   $wk_ket='';
           $sql2="SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM (
                    SELECT 1 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_100 WHERE NO_BARANG = '$hawb'
                    UNION ALL
                    SELECT 1 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_200 WHERE NO_BARANG = '$hawb'
                    UNION ALL
                    SELECT 1 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_300 WHERE NO_BARANG = '$hawb'
                    UNION ALL
                    SELECT 1 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_500 WHERE NO_BARANG = '$hawb'
                    UNION ALL
                    SELECT 1 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_501 WHERE NO_BARANG = '$hawb'
                    UNION ALL
                    SELECT 2 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_900 WHERE NO_BARANG = '$hawb'
					UNION ALL
                    SELECT 8 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_400 WHERE NO_BARANG = '$hawb'
                    UNION ALL
                    SELECT 9 as idx ,NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,WK_RESPON,create_at FROM dbclearance_gtln.bc_respone_403 WHERE NO_BARANG = '$hawb'
					
                    ) AS tabel ORDER BY WK_REKAM DESC ,idx DESC LIMIT 0,1";
         $result2 = $this->db->query($sql2);
		 foreach ($result2->result() as $row2){	
		 	$kdrespon = $row2->KD_RESPON;
            $wkrespon = $row2->KET_RESPON;
			$wk_ket .= $row2->WK_RESPON;
			$wk_time .=date('d-m-Y : H:i:s',strtotime($row2->WK_REKAM));
			//$wk_time=date('d-m-Y : H:i:s',strtotime($row2->WK_REKAM));
		 }
		...
		// if($wkrespon !=''){
				...
				$objPHPExcel->getActiveSheet()->setCellValue('W'.$kol, $wk_time);
				$objPHPExcel->getActiveSheet()->setCellValue('X'.$kol, $wk_ket);
				$kol++;
				$nomor++;
			 //  }
...
function release_excel($tanggal='',$tabel=''){	
...
//----------------------CREATE COLOUMN HEADER------------------------------------------------------------------------
		$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Release Manifest Date '.date('d M Y',strtotime($tanggal)));
		$objPHPExcel->getActiveSheet()->setCellValue('B3', 'No');
		$objPHPExcel->getActiveSheet()->setCellValue('C3', 'Hawb');
		$objPHPExcel->getActiveSheet()->setCellValue('D3',  'Mawb');
		$objPHPExcel->getActiveSheet()->setCellValue('E3',  'Flight Date');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('R3', 'Pph_persen');
		$objPHPExcel->getActiveSheet()->setCellValue('S3',  'Pph_value');
		$objPHPExcel->getActiveSheet()->setCellValue('T3',  'Total Tax');
		$objPHPExcel->getActiveSheet()->setCellValue('U3',  'Status');
		$objPHPExcel->getActiveSheet()->setCellValue('V3',  'Description');
       ...
       $result=$this->Model_gtln->getCustom("*",$tabel,
        ...
        foreach ($result as $row)
        {
           ...
           $sql2="SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM (
							SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM dbclearance_gtln.bc_respone_400 WHERE NO_BARANG = '$hawb'
							UNION ALL
							SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM dbclearance_gtln.bc_respone_401 WHERE NO_BARANG = '$hawb'
							UNION ALL
							SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM dbclearance_gtln.bc_respone_403 WHERE NO_BARANG = '$hawb'
							UNION ALL
							SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM dbclearance_gtln.bc_respone_405 WHERE NO_BARANG = '$hawb'
							UNION ALL
							SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM dbclearance_gtln.bc_respone_406 WHERE NO_BARANG = '$hawb'
							UNION ALL
							SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM dbclearance_gtln.bc_respone_408 WHERE NO_BARANG = '$hawb'
							UNION ALL
							SELECT NO_BARANG,KD_RESPON,KET_RESPON,WK_REKAM,create_at FROM dbclearance_gtln.bc_respone_409 WHERE NO_BARANG = '$hawb'
...
//		 foreach ($result2->result() as $row2){		  
		...
		// Rename worksheet name
		$objPHPExcel->getActiveSheet()->setTitle('Release Manifest ' .$tanggal);
		...
		// Redirect output to a client’s web browser (Excel2007)
		header('<a class="zem_slink" title="Internet media type" href="http://en.wikipedia.org/wiki/Internet_media_type" target="_blank" rel="wikipedia">Content-Type</a>: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="Release_Manifest_'.$tanggal.'.xlsx"');
		header('<a class="zem_slink" title="Web cache" href="http://en.wikipedia.org/wiki/Web_cache" target="_blank" rel="wikipedia">Cache-Control</a>: max-age=0');
		// If you're serving to <a class="zem_slink" title="Internet Explorer 9" href="http://windows.microsoft.com/ie" target="_blank" rel="homepage">IE 9</a>, then the following may be needed
		header('Cache-Control: max-age=1');
