...
function ManifestList_search(){
		...
		$pecah_1=explode("_____", $inputan);
        $pecah=str_replace("%20"," ",$pecah_1);
...
function ManifestList_search_job(){
		$inputan=$this->uri->segment(4);
        $pecah_1=explode("_____", $inputan);
        $pecah=str_replace("%20"," ",$pecah_1);
				$date1=$pecah[0];
			...
			$b=$this->session->userdata('ses_mydigit');
			$c=$this->session->userdata('ses_type_clear');
			$d=$this->session->userdata('ses_pmk_182');
			$stat='';
			$digit='';
			$tipe='';
			$pmk182='';
/*			for($i=0;$i<count($a);$i++){
		...
		$nm_coloum= array('b.mawb','a.hawb','b.flight_date','a.trackno','c.Keterangan as TypeName','e.ket','d.Keterangan','b.shipper_name','b.consignee_name','b.type_clearance','b.kindofGood','e.UR_JNS');
        $orderby= array('b.flight_date' => 'desc');
		$where= "b.flight_date >= '$date1' AND b.flight_date <= '$date2'";
		$tipe=$tipe;
		$where_in=$txtsearch;
        $list = $this->Model_cari->get_datatables2($selected,$nm_tabel,$nm_coloum,$orderby,$where,$where_in,$tipe,$kriteria,$job,$nm_tabel2,$kolom1,$kolom2);
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->Model_cari->count_all2($nm_tabel,$nm_coloum,$nm_tabel2,$kolom1,$kolom2),
						"recordsFiltered" => $this->Model_cari->count_filtered2($nm_tabel,$nm_coloum,$orderby,$where,$where_in,$tipe,$kriteria,$job,$nm_tabel2,$kolom1,$kolom2),
						"data" => $data,
				);
		//output to json format
		echo json_encode($output);
...
function ManifestList_search_job_assign(){
		$inputan=$this->uri->segment(4);
		$pecah_1=explode("_____", $inputan);
        $pecah=str_replace("%20"," ",$pecah_1);
				$date1=$pecah[0];
...
/*			for($i=0;$i<count($a);$i++){
		...
		$nm_coloum= array('b.mawb','a.hawb','b.flight_date','a.trackno','c.Keterangan as TypeName','e.ket','d.Keterangan','b.shipper_name','b.consignee_name','b.type_clearance','b.kindofGood','e.UR_JNS');
        $orderby= array('b.flight_date' => 'desc');
		$where= "b.flight_date >= '$date1' AND b.flight_date <= '$date2' AND f.id_user ='$usr'";
		$tipe=$tipe;
		$where_in=$txtsearch;
        $list = $this->Model_cari->get_datatables3($selected,$nm_tabel,$nm_coloum,$orderby,$where,$where_in,$tipe,$kriteria,$job,$nm_tabel2,$kolom1,$kolom2);
...
function ManifestList_search_assign(){
        ...
        $pecah=str_replace("%20"," ",$pecah_1);
                ...
                $smart_role=$this->Model_user->getCustom('Hawb',"assign_hawb","where Id_User= '$iduser'");
        //===TLC  
            $usrass ="AND b.hawb not in (";
            $usrass_nu=0;
            foreach($smart_role as $row){      
				if($usrass_nu==0){$usrass .= "'".$row->Hawb."'";}else{$usrass .= ",'".$row->Hawb."'";}
			$usrass_nu++;
            }
            $usrass .=")";
            if($usrass_nu==0){$usrass ="";}
       //===========================================
...
/*			for($i=0;$i<count($a);$i++){
		...
		$selected=array('b.mawb','a.hawb','b.flight_date','a.trackno','c.Keterangan as TypeName','e.ket','d.Keterangan','b.shipper_name','b.consignee_name','b.type_clearance','b.kindofGood','e.UR_JNS','d.CodeStatus','(SELECT COUNT(*) AS Sts FROM assign_hawb WHERE Hawb = a.hawb ) as Sts');
		$nm_coloum= array('b.mawb','(SELECT COUNT(*) AS Sts FROM assign_hawb WHERE Hawb = a.hawb )','a.hawb','b.flight_date','a.trackno','c.Keterangan','e.ket','d.Keterangan','b.shipper_name','b.consignee_name','b.type_clearance','b.kindofGood','e.UR_JNS','d.CodeStatus');
        $orderby= array('b.flight_date' => 'desc');
		$where= "b.flight_date >= '$date1' AND b.flight_date <= '$date2' and e.Id_aju in ('4','3') and b.TrackingStatus <> '37' $usrass ";
		$tipe=$tipe;
		$where_in=$txtsearch;
        $list = $this->Model_cari->get_datatables($selected,$nm_tabel,$nm_coloum,$orderby,$where,$where_in,$tipe,$kriteria,$job,$nm_tabel2,$kolom1,$kolom2);
		...
		foreach ($list as $row){
			$no++;
			$row = array(
            'Act' => '<a onclick="add_assign('."'".$row->hawb."'".')" data-uk-tooltip="{pos='."'right'".'}" title="ADD ASSIGN" href="javascript:void(0)" ><i class="material-icons">done</i></a>',
            'Sts' => '<a onclick="show_assign('."'".$row->hawb."'".')" href="javascript:void(0)" >'.$row->Sts.' User </a>',
            'Mawb' => $row->mawb,
            'Hawb' => $row->hawb,
            'FlightDate' => $this->Mdata->tanggal_format_indonesia($row->flight_date),
            'TrackingNo' => $row->trackno,
...
function NotInAssign_Job($pt="",$cwhere="",$ctgl1="",$ctgl2="",$idusr=""){
        $this->load->model('user/Model_user');
        
        
        $smart_role=$this->Model_user->getCustom('Hawb',"assign_hawb","where Id_User= '$idusr'");
        //===TLC  
            $usrass ="AND b.hawb not in (";
            $usrass_nu=0;
            foreach($smart_role as $row){      
       ...
       //===========================================
        ...
        $nm_coloum_order= array('b.mawb','(SELECT COUNT(*) AS Sts FROM assign_hawb WHERE Hawb = a.hawb )','a.hawb','b.flight_date','a.trackno','c.Keterangan','e.ket','d.Keterangan','b.shipper_name','b.consignee_name','b.type_clearance','b.kindofGood','e.UR_JNS','d.CodeStatus');

        $nm_coloum= array('b.mawb','a.hawb','b.flight_date','a.trackno','c.Keterangan as TypeName','e.ket','d.Keterangan','b.shipper_name','b.consignee_name','b.type_clearance','b.kindofGood','e.UR_JNS','d.CodeStatus','(SELECT COUNT(*) AS Sts FROM assign_hawb WHERE Hawb = a.hawb ) as Sts');
        ...
        $whereall=  array();
        //$where=  array('b.flight_date >= '=>date('Y-m-d'),'b.flight_date <= '=>date('Y-m-d'));
        $where=  "b.flight_date >= '$ctgl1' and b.flight_date <= '$ctgl2' and e.Id_aju in ('4','3') and b.TrackingStatus <> '37' $usrass";
        ...
        $where = $cwhere." and b.flight_date >= '$ctgl1' and b.flight_date <= '$ctgl2' and e.Id_aju in ('4','3') and b.TrackingStatus <> '37' $usrass";
        }
   ...
   //     $this->Mdata->myKill();
    ...
    function ShowUserAssign(){
        
         $id            = $this->input->post('hawb');        
        
         $q=$this->db->query("SELECT b.FullName,c.GroupName FROM assign_hawb a 
                                INNER JOIN msuser b ON a.Id_User=b.Id
                                INNER JOIN msgroup_usr c ON b.IdGroupUsr=c.Id
                                WHERE a.Hawb = '$id' ");
         $no = 0 ;
         $data = array();
    ...
    public function cmbStatusProses_filter()
	{
	    $tlc = $this->uri->segment(4);
	    $idgroup=$this->session->userdata('cs_Idgroup_usr');
        $nm_coloum= array('b.Noid','b.CodeStatus','b.Keterangan');
        $where=  array('a.Code1'=>$tlc,'GroupId'=>$idgroup);
        $orderby= array('b.Noid' => 'asc');
        $list = $this->Mdata->get_datadetail_join1($nm_coloum,'smart_role a','statusproses b','a.Code2=b.Noid','inner',$orderby,$where);
    ...
    function cek_StatusProses(){
        ...
        if($i>=2){
            if($tr[0] <> ''){
                ...
                $q=$this->db->query("SELECT COUNT(CodeStatus) AS jml FROM statusproses WHERE CodeStatus ='$tr[3]'");
                $ka=0;
                    foreach ($q->result() as $row)
                    {
                           $ka=$row->jml;
                           if($ka == 0){
                                $error .= " TLC Status Clearance ".$tr[3]." is not fount,";
                                $cek=1;
                           }
                    }
        ...
        foreach ($dataku as $rec){
           ...
           if ($id>=1){ 
            if($tr[1] <> ''){   
               ...
               $note        = $tr[4];  
                //=================//    
                    $q=$this->db->query("SELECT Noid,Keterangan FROM statusproses WHERE CodeStatus ='$tr[3]'");
                    foreach ($q->result() as $row)
                    {
                           $tlc=$row->Noid;
    ...
    function StatusProsesList($pt="",$cwhere="",$ctgl1="",$ctgl2=""){
        ...
        $nm_coloum= array('IdImport','ImportDate','UserId','UserName');
              
        $nm_tabel='hisimportstatusproses';
         
        
        $orderby= array('IdImport' => 'desc');
        
        $whereall=  array();
        $where=  array('date(ImportDate) >= '=>date('Y-m-d'),'date(ImportDate) <= '=>date('Y-m-d'));
        ...
        $where = $cwhere." and ImportDate >= '$ctgl1' and ImportDate <= '$ctgl2'";
        }
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->Mdata->count_all($nm_tabel,$nm_coloum),
...
//        $noawal1=$extgl[0].$extgl[1].$extgl[2];
    ...
    public function discuctionAdd(){
        ...
        $noawal1=$extgl[0].$extgl[1].$extgl[2];
        $idticket = $this->input->post('idticket');
/*-- followed comment to all user with job list clause for notification */
        $statustracking = $this->input->post('statustrack');
        $type_clearance = $this->input->post('tipeclear');
        $pmk_182        = $this->input->post('pmk182');
        $hawb           = $this->input->post('hawb');
		$lastdigit=substr($hawb, -1);
		$allowedTo='';                          
		$cari_usr=$this->Model_cari->getCustom("b.Id",
			  "smart_role a",
			  "INNER JOIN msuser b ON a.GroupId=b.IdGroupUsr 
                ...
                INNER JOIN msgroup_usr f ON b.IdGroupUsr=f.Id 
				WHERE a.Code1='$statustracking' AND (d.type_clearance='$type_clearance' OR d.pmk_182='$pmk_182') AND e.id_alpha = '$lastdigit'
                GROUP BY b.Id");
					foreach($cari_usr as $dt){
						$allowedTo .='|'.$dt->Id.'|';
					}
/*--  followed comment to all user with job list clause */
    ...
    public function assignRemove(){
        $id     = $this->input->post('idhawb');
        $idusr  = $this->input->post('idusr');
        
        $where = array('Id_User'=>$idusr,'Hawb'=>$id);
        
        $this->Mdata->delete_where($where,'assign_hawb','Hawb');
        echo json_encode(array('sts'=>true));
    }
    ...
    public function StatusTrackingUpdate(){
                ...
                $idsts = $this->input->post('ut_statusproses');    
                $q=$this->db->query("SELECT * FROM statusproses WHERE trigger_ticket = '1' AND Noid = '$idsts'");
                $ka=0;
                ...
                if($ka==1){
                    $this->db->query("UPDATE ticket_full_hdr SET status_id = '3' WHERE object_id = '$hawb'"); 
                }
        ...
        public function AttachmentGet(){
    		...
    		foreach ($list as $row){
                ...
                'Action'=> '<a class="green" href="javascript:void()" data-uk-tooltip  title="Download '.$row->AttachmentName.'" onclick="download_attachment('."'".$row->IdAttachment."'".','."'".$row->AttachmentName."'".')"><i class="material-icons md-24 uk-text-primary">cloud_download</i></a>&nbsp;&nbsp;
                            <a class="blue" href="javascript:void()" data-uk-tooltip  title="Preview '.$row->AttachmentName.'" onclick="open_attachment('."'".$row->AttachmentName."'".','."'".$row->AttachmentName."'".')"><i class="material-icons md-24 uk-text-success">remove_red_eye</i></a>&nbsp;&nbsp;
...
// ======== ROLE GROUP STATUS TRACKING =========
     ...
     function ManifestList_myjob($pt="",$cwhere="",$ctgl1="",$ctgl2=""){
        ...
        $whereall=  array();
        //$where=  array('b.flight_date >= '=>date('Y-m-d'),'b.flight_date <= '=>date('Y-m-d'));
        $where=  "RIGHT(b.hawb,1) in $digit and b.TrackingStatus in $tlc and (  b.Id_aju in $pmk182 ) and b.flight_date >= '$ctgl1' and b.flight_date <= '$ctgl2'";
        ...
        $where = $cwhere." and RIGHT(b.hawb,1) in $digit and b.TrackingStatus in $tlc and (  b.Id_aju in $pmk182 ) and b.flight_date >= '$ctgl1' and b.flight_date <= '$ctgl2'";
        }
     ...
     function ManifestList_myjob_assign($pt="",$cwhere="",$ctgl1="",$ctgl2=""){
        //========================================
        ...
        $whereall=  array();
        //$where=  array('b.flight_date >= '=>date('Y-m-d'),'b.flight_date <= '=>date('Y-m-d'));
        $where=  " b.TrackingStatus in $tlc AND f.Id_User ='$usr' and b.flight_date >= '$ctgl1' and b.flight_date <= '$ctgl2'";
        ...
        $where = $cwhere." b.TrackingStatus in $tlc AND f.Id_User ='$usr' and  b.flight_date >= '$ctgl1' and b.flight_date <= '$ctgl2'";
        }
        ...
        $list = $this->Mdata->get_datatables_join5($nm_tabel,$nm_coloum,$nm_tabel1,'inner',$on1,$nm_tabel2,'left',$on2,$nm_tabel3,'left',$on3,$nm_tabel4,'left',$on4,$nm_tabel5,'inner',$on5,$orderby,$where,$nm_coloum_order);
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" =>  $this->Mdata->count_all_join5($nm_tabel,$nm_coloum,$nm_tabel1,'inner',$on1,$nm_tabel2,'left',$on2,$nm_tabel3,'left',$on3,$nm_tabel4,'left',$on4,$nm_tabel5,'inner',$on5,$where),
						"recordsFiltered" => $this->Mdata->count_filtered_join5($nm_tabel,$nm_coloum,$nm_tabel1,'inner',$on1,$nm_tabel2,'left',$on2,$nm_tabel3,'left',$on3,$nm_tabel4,'left',$on4,$nm_tabel5,'inner',$on5,$orderby,$where),
						"data" => $data,
				);
		//output to json format
		echo json_encode($output);
    ...
    function InAssign_Job($usr="",$cwhere="",$ctgl1="",$ctgl2="",$pt=""){
        //========================================
        ...
        $whereall=  array();
        //$where=  array('b.flight_date >= '=>date('Y-m-d'),'b.flight_date <= '=>date('Y-m-d'));
        $where=  " b.TrackingStatus <> '37' AND f.Id_User ='$usr' ";
        ...
        $where = $cwhere." b.TrackingStatus <> '37' AND f.Id_User ='$usr' ";
        }
		...
		foreach ($list as $row){
			$no++;
			$row = array(
            'Act' => '<a onclick="remove_assign('."'".$row->hawb."'".')" data-uk-tooltip="{pos='."'right'".'}" title="REMOVE ASSIGN" href="javascript:void(0)" ><i class="material-icons">clear</i></a>',
            'Mawb' => $row->mawb,
            'Hawb' => $row->hawb,
            'FlightDate' => $this->Mdata->tanggal_format_indonesia($row->flight_date),
            'TrackingNo' => $row->trackno,
...
//              'smtp_pass' => 'muharami', 
    ...
    function get_barangkiriman(){
            $hid=$this->uri->segment(4);
            $url     = 'http://116.206.197.135/get_response_bc/bkresponse/Cbkresponse/get_response';
            $myvars  = 'sts=1&data_hawb='.$hid.'&npwp=020640181428000';
            $ch      = curl_init( $url );
            ...
            $cRet ='<br/>
                    <h3 class="heading_a"> Response Barang Kiriman </h3>
                    <hr/>';
            $cRet .='<div class="uk-text-left">
                        <a onclick="sinkronisasi()" class="md-btn md-btn-small md-btn-success waves-effect waves-button ">
                        <i class="md-list-addon-icon material-icons md-24 md-color-white">refresh</i> Synchronize Barang Kiriman</a>
                   </div>';            
            $cRet .='<table id="qwerty" class="uk-table uk-table-hover" cellspacing="0" width="100%">
                        <thead>
                        <tr>
                            <th>TGL_REKAM</th>
                            <th>JAM_REKAM</th>                            
    ...
    function sinkronisasi(){
            ...
            $bb=json_decode($response);
                    ...
                    foreach ($bb as $row4)
                    {
                       ...
                       $pecahtgl = explode('-',$b);
                       $pecahwk = explode(':',$c);
                       $noawal = $pecahtgl[0].$pecahtgl[1].$pecahtgl[2].$pecahwk[0].$pecahwk[1].$pecahwk[2].$a.$d;
                       $this->db->from('dstatusproses');
                       $this->db->where(array('Hawb'=>$hid,'IdStatusProses'=>$d));
                       ...
                       if($jml==0){
                        ...
                        $q=$this->db->query("SELECT DateUpdate,IdStatusProses,Note  FROM dstatusproses WHERE Hawb ='$hid' ORDER BY DateUpdate DESC ,Id DESC LIMIT 0,1");
                        foreach ($q->result() as $row)
                        {
                            $nmtabel_='t_shipment_cloud';
                            $key_='hawb';
                        	...
                        	$this->Mdata->update(array($key_ => $hid), $data_,$nmtabel_);
                        }
    ...
    function kt(){
        ...
        $noi=0;
                     $q=$this->db->query("SELECT * FROM t_shipment_cloud WHERE TrackingStatus = ' '");
                        foreach ($q->result() as $rows)
                        {
                            $hawb = $rows->hawb;
                            $q=$this->db->query("SELECT IdStatusProses FROM dstatusproses WHERE Hawb='$hawb' ORDER BY DateUpdate DESC LIMIT 0,1");
                            foreach ($q->result() as $row)
                            {
                                $nmtabel_='t_shipment_cloud';
                                $key_='hawb';
    ...
    function close_ticket(){
        ini_set('max_execution_time', 1000);
        $noi=0;
                     $q=$this->db->query("SELECT * FROM t_shipment_cloud WHERE TrackingStatus IN ('37','64','65','408') AND hawb IN (SELECT object_id FROM ticket_full_hdr)");
                        foreach ($q->result() as $rows)
                        {
                            $hawb = $rows->hawb;
                            $this->db->query("UPDATE ticket_full_hdr SET status_id = '3' WHERE object_id = '$hawb' "); 
    ...
    function rafles_(){
                      $q=$this->db->query("SELECT * FROM t_shipment_cloud WHERE TrackingStatus = '68'");
                      $noi=0;
                        foreach ($q->result() as $rows)
                        {
                            $hawb = $rows->hawb; 
