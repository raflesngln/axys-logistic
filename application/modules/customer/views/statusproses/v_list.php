...
.uk-tab > li.uk-active > a {
 ...
 "lengthMenu": [[10, 60, 100, -1], [10, 60, 100, "All"]],
            ...
            "ajax": {
                "url": "<?php echo site_url('cas/Statusproses/listdata')?>",
                "type": "POST",
            },
            "columns": [
                        ...
                        <thead>
                            <tr>
                              <th width="2%">No</th>
                                ...
                                <th width="5%">IdFolingData</th>
                                <th width="5%">FolingData</th>
                                <th width="5%">#</th>
                            </tr>
                        </thead>
                       <tbody>
...
<form method="post" action="javascript:void(0);" onsubmit="save_status()" id="form_status" enctype="multipart/form-data" data-parsley-validate="">
...
<div class="md-card-content">
  ...
  <div class="uk-width-medium-1-2">
    ...
    <input name="CodeStatus" type="text" class="md-input label-fixed" id="CodeStatus" required="required" />
    <span class="md-input-bar"></span>
         ...
         <div class="uk-width-medium-1-1">
                          <div class="uk-form-row">
                                <div class="uk-grid">
                                  <div class="uk-width-medium-1-2">
                                    <label>FolingData</label>
                                    <textarea name="FolingData" class="md-input label-fixed" required="required" id="FolingData"></textarea>
...
function new_status(){
	...
	modal.show();
	$("#form_status").attr('onsubmit','save_status()');
...
function edit_data(mydata){
	...
	var id=mydata;
	       $.ajax({
		   type: "POST",
           url : "<?php echo site_url('cas/Statusproses/load_edit_data')?>",
		    data: "id="+id,
           dataType: "json",
           success: function(data){
			   swal.close();
			   	modal=UIkit.modal("#modal_status",{bgclose:false,modal:false});
				modal.show();
				$("#form_status").attr('onsubmit','update_status()');
                for (var i =0; i<data.length; i++){
                   $("#CodeStatus").val(data[i].CodeStatus);
				   $("#FolingData").val(data[i].FolingData);
				   $("#remarks").val(data[i].Keterangan);
				   $("#idstatus").val(data[i].Noid);
...
function update_status(){
   // var t = JSON.stringify($('#myform').serialize());
          swal_process();
          url = '<?php echo base_url();?>cas/Statusproses/update_status';
          $.ajax({
            url : url,
            type: "POST",
...
function save_status(){	
       ...
       // ajax adding data to database
          $.ajax({
            url : "<?php echo site_url('cas/Statusproses/save_status')?>",
            type: "POST",
...
function nonactive_status(mydata){
    ...
    //Logic to delete the item
	       $.ajax({
		   type: "POST",
           url : "<?php echo site_url('cas/Statusproses/nonactive_status')?>",
		    data: "id="+id,
           dataType: "json",
           success: function(data){
