...
$(document).ready(function(){
    ...
    var
      data = [
        {Hawb: 'Hawb',Date : 'Modify Date (M/D/YYYY)',Time : 'Modify Time (HH:mm:ss)' ,TLC : 'TLC Status Tracking',Note : 'Remark'}];
...
//      }
    ...
    //  $('#button_import').hide();  
      ...
      UIkit.modal.confirm('Are you sure update status Shipment '+ctxtstsproses+' ?', function(){
      ...
      swal({title: "Prosesing...!",text: '<center><div class="proses_loader"></div></center>',html: true,showConfirmButton: false});    
        ...
        AddItems('Cek Data...!','','0');
        $.ajax({
            url: "<?php echo base_url(); ?>/cas/Cas/cek_StatusProses",
            data: {
                "data": mydata
                			    ...
                			    var cek=true;
                                 ...
                                 if(cek){
                                  AddItems('Please wait, Proses Import Data...!','','0');   
                                    $.ajax({
                                        type: "POST",
                        				url: '<?php echo base_url(); ?>cas/Cas/UpdateStatusProses',
                                        data:{
...
list = $("#list").DataTable({
                ...
                "order": [[ 2, "desc" ]],
                "ajax": {
                    "url": "<?php echo site_url('cas/Cas/StatusProsesList')?>",
                    "type": "POST",  
                },
                "columns": [
                { "data": "IdImport"},
...
function cari(){
      ...
      //  alert(cfie+'  '+ceve+' '+cval);
        ...
        if (cfie==''){var cwhere='';}
        list.ajax.url( "<?php echo base_url(); ?>"+"cas/Cas/StatusProsesList/"+zpt+"/"+cwhere+"/"+date1+"/"+date2 ).load();    
    }
    
function print_(id='',ctk=''){
    switch (ctk) {
    case '1':
        cetakPrev('4|'+id,"<?php echo site_url('cas/Crptcas/rpt_ImportStatusProses')?>","#preview_his"); 
        UIkit.switcher('#h_tab').show(2);
        break;
    case '2':
    alert(ctk);
        cetakTri('1|'+id,"<?php echo site_url('cas/Crptcas/rpt_ImportStatusProses')?>");     
        break;
    case '3':
    alert(ctk);
        cetakTri('2|'+id,"<?php echo site_url('cas/Crptcas/rpt_ImportStatusProses')?>"); 
        break;
    }
}
    ...
    <div class="uk-width-medium-1-1">
        <div class="md-card" id="cont">
            <div class="md-card-toolbar" style="height: 55px;">
                ...
                <h3 id="title" class="md-card-toolbar-heading-text">IMPORT STATUS PROSES</h3>
            </div>
    ...
    <!-- TAB SATU -->
                    ...
                    <br />
                        ...
                        <div class="uk-width-large-1-2" id="Sts">
                         <div style="display: none;" class="uk-form-row parsley-row">
                           <label for="">Status Proses<span class="req"> *</span></label>
