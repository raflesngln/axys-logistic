...
$(document).ready(function(){
	...
	//choioce all job or detail
	var hidden_text=$("#klik").val();
	if(hidden_text==''){
		load_mylist_job();
	} else {
		//trigger clik on Id shipment for detail
			jQuery(function(){
				jQuery('#klik').click();
			});	
	}  
          ...
          $.ajax({
            ...
            success: function(data)
            {
                ...
                clearanceArr.forEach(function(element) {
                    if(element.id==data.type_clearance){$('.v_TypeClerance').html(element.name)};
                });     
                
                $('.v_aju').html('');
                ajuArr.forEach(function(element) {
                    if(element.id==data.Id_aju){$('.v_aju').html(element.name)};
                }); 
...
/*                                myhtml = '<tr>'+
								...
								var mydata2='<div class="uk-width-3-10" style="border-right: 2px #ddd solid;">'
										+no+'. '+data[i].TrackingNo+' - '+data[i].billterm
										+'</div>';
...
function load_mylist_job(){  
	var id=id;
       $.ajax({
		   type: "POST",
           url : "<?php echo site_url('cas/Ticket/load_data_job')?>",
		    data: "id="+id,
           dataType: "json",
           success: function(data){
                    $("#val").empty();
                     for (var i =0; i<data.length; i++){
                   var option =data[i].complain_name+'\n';
                          $("#val").append(option);
                       }
               }
       }); 
...
function cari2(){
		...
		var txtcari=input[0]+'_'+input[1]+'_'+input[2]+'_'+input[3]+'_'+input[4]+'_'+input[5]+'_'+input[6]+'_'+input[7]+'_'+input[8]+'_'+input[9];
		var parameter=date1+"_"+date2+"_"+tipeclearance+"_"+kriteria+"_"+txtcari;
        ...
        <div class="uk-grid" >
            ...
            <div class="uk-width-medium-1-4 uk-width-large-1-4">
               <select name="fie" id="fie" data-md-selectize>
                  ...
                  <option value="e.ket"> TYPE CLEARANCE PMK 182</option>
               </select>
             </div>
             <div class="uk-width-medium-1-4 uk-width-large-1-4">
             ...
             <div class="uk-width-medium-1-5 uk-width-large-1-5">
               ...
               <input type="text" id="klik" onclick="konfir(this)" value="<?=$awb;?>" />
             </div>
        </div>
        <br />
        <br />
        ...
        <div class="uk-width-large-1-3">
          ...
          <div class="uk-width-large-1-3">
             <ul class="md-list">
                <li>
                   <div class="md-list-content">
                      <span class="uk-text-small uk-text-muted uk-display-block">Type Clearane Base On PMK 182</span>
