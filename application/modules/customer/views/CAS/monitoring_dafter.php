...
//         colVis_set(list,$("#list_inquiry"));
               ...
               list = $('#list').DataTable({
                      ...
                      "columns": [
                        ...
                        { "data": "Group"},
                        { "data": "Target"},
                        { "data": "Actual","class":"uk-hidden"},
                        { "data": "Selisih","class":"uk-hidden"},
                        { "data": "Persen","class":"uk-hidden"},
                        ]
                });
...
function load_detail(){    
  var ctgl = $('#uk_dp_1').val();                     
          ...
          $('.MyContent_1').hide();
                       $.ajax({
                            url: "<?php echo site_url('cas/Ccas_rpt/Monitoring_Dafter')?>",
                            type:"POST",
                            data:({tgl:ctgl}),
                            dataType: "json",
  ...
  //  alert(idgroup+'-'+id+'-'+sts);
      ...
      var ctgl = $('#uk_dp_1').val();                     
          ...
          $('.MyContent_1_h').hide();
                       $.ajax({
                            url: "<?php echo site_url('cas/Ccas_rpt/Detile_Monitoring_Dafter')?>",
                            type:"POST",
                            data:({tlc:tlc,cid:id}),
                            dataType: "json",
                            success: function(data) {
                             for (var i =0; i<data.length; i++){
                                    listhawb.row.add({
                                    "No"            : data[i].No,    
                                    "Hawb"          : '<a onclick="getlink_notif('+"'<?php echo site_url('cas/Ccas/ManifestList_view');?>?idhawb="+data[i].Hawb+"'"+')">'+data[i].Hawb+'</a>',
    ...
    <!-- MODAL UPDATE STATUS -->                
       <div class="uk-modal" id="modal_detail_hawb">
            <div class="uk-modal-dialog">
              ...
              <div style="max-height: 300px;" class="uk-overflow-container">
