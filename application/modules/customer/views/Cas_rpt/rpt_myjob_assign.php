...
function print_(ctk=''){
    var track   = <?php echo $this->session->userdata('cs_Idusr'); ?>;
    if(track ==   ' '){
         swal("Oops... Status User Name is required", "Please Input User Name!...", "error");
         exit();
    }
    ...
    switch (ctk) {
    case '1':
        cetakPrev('4|'+tgl1+'|'+tgl2+'|'+track+'|'+$per+'|'+$sts,"<?php echo site_url('cas/Ccas_rpt/rpt_user_assignt')?>","#preview_his"); 
        UIkit.switcher('#h_tab').show(1);
        break;
    case '2':
    //alert(ctk);
        cetakTri('1|'+tgl1+'|'+tgl2+'|'+track+'|'+$per+'|'+$sts,"<?php echo site_url('cas/Ccas_rpt/rpt_user_assignt')?>");     
        break;
    case '3':
    //alert(ctk);
        cetakTri('2|'+tgl1+'|'+tgl2+'|'+track+'|'+$per+'|'+$sts,"<?php echo site_url('cas/Ccas_rpt/rpt_user_assignt')?>"); 
        break;
    }
}
    ...
    <div class="uk-width-medium-1-1">
        <div class="md-card" id="cont">
            <div class="md-card-toolbar" style="height: 55px;">
                ...
                <h3 id="title" class="md-card-toolbar-heading-text">REPORT MY JOB PENDING</h3>
            </div>
...
<!-- LISTING HISTORY IMPORT -->                    
        ...
        <div class="uk-grid" >
             <div class="uk-width-medium-1-2 uk-width-large-1-2 "> 
                 <span class="icheck-inline">
                      <input type="radio" name="radio_demo_inline1" id="radio_demo_inline_3" onchange="perval_sts(1);" checked="true"/>
                      <label for="radio_demo_inline_3" class="inline-label">ALL</label>
                 </span>
                 <span class="icheck-inline">
                      <input type="radio" name="radio_demo_inline1" id="radio_demo_inline_4" onchange="perval_sts(2);"/>
                      <label for="radio_demo_inline_4" class="inline-label">HOLD SHIPMENT</label>
                 </span>
                 <span class="icheck-inline">
                      <input type="radio" name="radio_demo_inline1" id="radio_demo_inline_5" onchange="perval_sts(3);"/>
                      <label for="radio_demo_inline_5" class="inline-label">REALASE SHIPMENT</label>
                 </span>
             </div>
        </div>
          <div class="uk-grid" style="display:none;">
            <div class="uk-width-large-1-3" id="Sts">
                <div class="uk-form-row parsley-row">
                    ...
                    <select id="username" name="username" required multiple>
                       <option value=" ">...</option>
                    </select>
                </div>
            </div>
