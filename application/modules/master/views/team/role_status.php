...
<div class="uk-form-row">
    ...
    <div class="uk-width-medium-1-1" id="box-role"> <?php echo $this->load->view('master/team/role_content');?> </div>
  </div>
</form>
...
function pindah1(idno){
	...
	var idgrup=$("#idgrup").val();
		...
		UIkit.modal.alert('<i class="material-icons md-24 md-color-red-A700">highlight_off</i> Please Choose Group !');
				...
				$.ajax({
                type: "POST",
                url : "<?php echo base_url('master/team/add_role'); ?>",
...
function pindah2(idno){
	...
	if(idgrup==''){
		UIkit.modal.alert('<i class="material-icons md-24 md-color-red-A700">highlight_off</i> Please Choose Group !');
				...
				$.ajax({
                type: "POST",
                url : "<?php echo base_url('master/team/remove_role'); ?>",
                data: "idno="+idno,
				data: "idno="+idno+"&idgrup="+idgrup,
                success: function(data){
					swal.close();
...
function load_role_group(){
	...
	if(idgrup==''){
			...
			$("#nonaktif").html('list role');
				...
				$.ajax({
                type: "POST",
                url : "<?php echo base_url('master/team/load_role_group'); ?>",
                data: "idgrup="+idgrup,
                success: function(data){
					swal.close();
					$("#box-role").html(data);
...
function load_group_list(){
	var id=id;
       $.ajax({
           ...
           success: function(data){
                    $("#idgrup").empty();
                   $("#idgrup").append("<option value=''>Select Team.....</option>");
