...
<div class="uk-form-row">
    ...
    <div class="uk-width-medium-1-1" id="box-role"> <?php echo $this->load->view('master/type_clearance/role_content');?> </div>
  </div>
</form>
...
/*  $(".idmn1").on("click", function(e) {
	 ...
	 alert(nil);
/*var text='<li><input type="checkbox" name="idno[]" class="idmn" id="'+nil+'" value="'+nil+'"  onclick="pindah1(this)"/><input type="hidden" class="myid2" value="'+nil+'" />'+title+'</li>';
	 // var text='<li><input type="checkbox" name="idno[]" class="idmn" id="'+nil+'" value="'+nil+'"  onclick="pindah2(this)"/>'+title+'</li>';
	  $("#aktif").append(text);
        e.preventDefault();
        $(this).parent().remove();
        x--;
...
function pindah1(idno){
	...
	var idgrup=$("#idgrup").val();
		...
		UIkit.modal.alert('<i class="material-icons md-24 md-color-red-A700">highlight_off</i> Please Choose Group !');
				...
				$.ajax({
                type: "POST",
                url : "<?php echo base_url('master/Type_clearance/add_role'); ?>",
...
function pindah2(idno){
	...
	if(idgrup==''){
		UIkit.modal.alert('<i class="material-icons md-24 md-color-red-A700">highlight_off</i> Please Choose Group !');
				...
				$.ajax({
                type: "POST",
                url : "<?php echo base_url('master/Type_clearance/remove_role'); ?>",
                data: "idno="+idno,
				data: "idno="+idno+"&idgrup="+idgrup,
                success: function(data){
					//swal.close();
...
/*		var idno=document.getElementsByName('idno[]');
...
function load_role_group(){
	...
	if(idgrup==''){
			...
			$("#nonaktif").html('list role');
				...
				$.ajax({
                type: "POST",
                url : "<?php echo base_url('master/Type_clearance/load_role_group'); ?>",
                data: "idgrup="+idgrup,
                success: function(data){
					swal.close();
					$("#box-role").html(data);
...
function load_group_list(){
	var id=id;
       $.ajax({
		   type: "POST",
           url : "<?php echo site_url('master/Type_clearance/load_group_list')?>",
		    data: "id="+id,
           dataType: "json",
           success: function(data){
                    $("#idgrup").empty();
                   $("#idgrup").append("<option value=''>Select User.....</option>");
                     for (var i =0; i<data.length; i++){
